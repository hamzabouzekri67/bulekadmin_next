...
export function QuantitySelector({
  ...
  const [defaultSupplementQty, setDefaultSupplementQty] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    ...
    if (listProducts?.listSuplement) {
      currentChooseList = JSON.parse(JSON.stringify(listProducts.listSuplement));
    }
  ...
  const increaseSupplement = (item: SupplementItem, chose: string) => {
    ...
    if (index !== -1) {
      newList[index] = { ...newList[index], qty: (newList[index].qty || 0) + 1 };
    } else {
      newList.push({
        id: item.id,
    ...
    setSupplementQty((prev) => ({ ...prev, [item._id]: (prev[item._id] || 0) + 1 }));

    const supplementsPrice = newList.reduce((sum, sup) => sum + Number(sup.price) * (sup.qty || 0), 0);
  ...
  const decreaseSupplement = (item: SupplementItem, chose: string) => {
    ...
    setChooseList(newList);
    setSupplementQty((prev) => ({ ...prev, [item._id]: Math.max((prev[item._id] || 1) - 1, 0) }));
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b shrink-0">
              <h2 className="text-base font-black text-gray-800">اختر الإضافات</h2>
              <button onClick={handleCloseBottomSheet} className="p-1 hover:bg-gray-100 rounded-full">
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-4 touch-pan-y bg-gray-50">
              {product.supplements && product.supplements.length > 0 ? (
                product.supplements.map((sup: Supplement, i: number) => (
                  <div key={i} className="bg-white p-3 rounded-2xl border border-gray-100 shadow-sm space-y-3">
                    <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                      <p className="font-black text-sm text-gray-800">{sup.title}</p>
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                        sup.chose === "requis" ? "bg-red-50 text-red-500" : "bg-gray-100 text-gray-600"
                      }`}>
                        ...
                        >
                          <div className="text-right flex-1 min-w-0">
                            ...
                            <span className="text-xs font-bold text-orange-600">
                              {Number(item.plusPrice) > 0 ? `+${item.plusPrice} ${product.currency}` : "مجاني"}
                            </span>
                          </div>
            ...
            <div className="p-4 border-t bg-white shrink-0">
              <button
                onClick={
                  isRequis
                    ? () => {
                        updateCart(product._id, quantity, product, chooseList, category);
                        setOpenedProductId(null);
                      }
                    : undefined
                }
              ...
              >
                {isRequis ? `تحديث السلة (${showTotal.toLocaleString()} ${product.currency})` : "يرجى تحديد الخيارات الإجبارية"}
              </button>
            </div>
          </div>
        </>
...
export function FloatingCart({ cartItems, order }: FloatingCartProps) {
  ...
  const totalItems = cartItems.reduce((a: number, i: CartItem) => a + (i.qentity || 0), 0);
  const totalPrice = cartItems.reduce((a: number, i: CartItem) => a + (i.totalprice || 0), 0);
        ...
        <div className="bg-gray-900 text-white p-3 rounded-2xl shadow-2xl flex justify-between items-center backdrop-blur-md bg-opacity-95">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="text-right min-w-0 flex-1">
              <p className="text-[9px] uppercase opacity-55 font-black tracking-wider">إجمالي الطلب</p>
        ...
        <div className="fixed inset-0 z-[110] flex items-end justify-center">
          ...
          <div className="relative w-full max-w-md bg-white rounded-t-3xl p-4 flex flex-col max-h-[82vh] pb-safe">
            ...
            <div className="flex justify-between items-center pb-3 border-b shrink-0">
              <h2 className="font-black text-base text-gray-800">تفاصيل الطلب</h2>
              <button onClick={() => setOpen(false)} className="text-gray-400 font-bold p-1 hover:bg-gray-100 rounded-full">
                ✕
              </button>
            </div>
                  ...
                  <div key={index} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                    <div className="flex justify-between gap-4 font-black text-sm text-gray-800">
                      <span className="flex-1 text-right line-clamp-1 min-w-0">
                        {item.title} <span className="text-orange-500">× {item.qentity}</span>
                      </span>
                      <span className="text-gray-700 shrink-0">{(item.totalprice || 0).toLocaleString()} DZD</span>
                    </div>
                        ...
                        {sups.map((sup, i) => {
                          ...
                          return (
                            <div key={i} className="flex justify-between items-center gap-2">
                              <span className="line-clamp-1 flex-1">• {sup.title} <span className="font-black text-gray-800">({sup.qty})</span></span>
                              <span className="font-semibold text-gray-500 shrink-0">+{lineTotal.toLocaleString()} DZD</span>
                            </div>
                          );
                        })}
                      </div>
            ...
            <div className="pt-3 border-t bg-white shrink-0 space-y-3">
              <div className="flex justify-between font-black text-base text-gray-900 px-1">
                ...
                <span className="text-orange-600">{totalPrice.toLocaleString()} DZD</span>
              </div>

              <button
                onClick={() => UpdateOrder({ order: order as Order | null, cartItems, router })}
