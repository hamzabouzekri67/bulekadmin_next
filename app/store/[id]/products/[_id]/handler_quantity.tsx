...
export function QuantitySelector({
  ...
  const handleUpdate = (newQty: number, isdec: boolean) => {
    ...
    if (newQty === 0) {
      ...
      setIsRequis(false);
      updateCart(product._id, newQty, product, chooseList, category);
  ...
  const handleCloseBottomSheet = () => {
    ...
    if (listProducts) {
      listProducts.listSuplement = JSON.parse(
        JSON.stringify(defaultChooseList),
      );
    }
                ...
                <div key={i}>
                  ...
                  <div className="space-y-3">
                    {sup.data.map((item: SupplementItem) => (
                      <div
                        key={item._id}
                        className="flex items-center justify-between p-3 border rounded-xl hover:bg-orange-50 transition"
                      >
                        ...
                        <div className="flex items-center gap-2 bg-gray-100 rounded-full px-2 py-1">
                          <button
                            ...
                            className="w-7 h-7 flex items-center justify-center rounded-full bg-white shadow text-gray-600"
                          >
                            −
                          </button>
                          ...
                          <button
                            ...
                            className="w-7 h-7 flex items-center justify-center rounded-full bg-orange-500 text-white shadow"
                          >
                            +
                          </button>
                        </div>
                ...
                onClick={
                  isRequis
                    ? () => {
                        ...
                        setDefaultSupplementQty({ ...supplementQty });
                        setDefaultQuantity(quantity);
                        setDefaultChooseList(
                          JSON.parse(JSON.stringify(chooseList)),
                        );
                        setOpenedProductId(null);
                      }
                    : undefined
...
export interface FloatingCartProps {
 ...
 export function FloatingCart({ cartItems, order }: FloatingCartProps) {
  ...
  // تصحيح النوع إلى number وتحديد نوع العنصر الحالي i ليكون CartItem
    ...
    <>
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md z-40">
        <div className="bg-gray-900 text-white p-4 rounded-2xl shadow-2xl flex justify-between items-center">
          ...
          <button
            ...
            className="bg-orange-500 hover:bg-orange-600 px-5 py-2.5 rounded-xl font-black text-sm shadow-lg transition-all active:scale-95"
          >
            Commander
          </button>
        </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          />
          <div className="relative w-full max-w-md bg-white rounded-t-3xl p-4">
                ...
                return (
                  <div key={index} className="bg-gray-50 rounded-xl p-3 border">
                    <div className="flex justify-between font-black">
                      ...
                      <span>{(item.totalprice || 0).toLocaleString()} DZD</span>
                    </div>
            ...
            <button
              onClick={() =>
                UpdateOrder({ order: order as Order | null, cartItems, router })
              }
