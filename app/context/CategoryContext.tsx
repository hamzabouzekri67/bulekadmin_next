...
interface CategoryContextType {
  ...
  cartItems: CartItem[];
  isCategoryDeletable: (categoryId: string) => boolean; // دالة جديدة
}
...
const CategoryContext = createContext<CategoryContextType>({
  ...
  cartItems: [],
  isCategoryDeletable: (categoryId: string) => false,
});
...
export function CategoryProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    setproduct((prev) => {
      ...
      order.listOrder.forEach((item) => {
        ...
        const exists = newList.find((ci) => ci.idproducts === item.idproducts);
  ...
  const isCategoryDeletable = (categoryId: string) => {
