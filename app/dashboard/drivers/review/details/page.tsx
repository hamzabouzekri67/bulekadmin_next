...
import { GetDriverDetailes } from "../../details/api/GetDetailesDriver";
import { useUser } from "@/app/context/UserContext";
import { PaymentInfo } from "@/app/types/PaymentInfo";
import { DriverData } from "@/app/types/Drivers";
import Image from "next/image";
import { updateStatusDriver } from "./api/handel_driver";
...
const DriverReviewPage = () => {
  ...
  const handleAction = async (action: "approve" | "reject") => {
    const driverId = driverDetailes?._id as string;
    if (!driverId) return;
    ...
    try {
      ...
      // 1. إرسال طلب التحديث إلى الـ API
      const response = await updateStatusDriver(driverId);

      // 2. التحقق من نجاح عملية التحديث بأمان
      if (response && response.status === true) {
        // الجلب الذكي للحالة سواء كانت من الكائن المباشر أو من داخل الـ result
        const currentStatus = response.result?.status || response.status_driver;
        setDriverDetailes(response.result);
        if (currentStatus === "completed") {
          alert("تم تفعيل وتأكيد حساب السائق بنجاح! 🚀✅");
          router.push(`/dashboard/drivers/details?id=${driverId}`);
        } else if (currentStatus === "rejected") {
          alert("تم رفض ملف السائق بنجاح. ❌");
        } else {
          router.push("/dashboard/drivers");
        }
      } else {
        alert(response.message || "فشلت العملية، يرجى المحاولة لاحقاً");
      }
    } catch (error) {
      console.error("Erreur lors de la mise à jour du chauffeur:", error);
      alert("حدث خطأ في الاتصال بالسيرفر ⚠️");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 md:p-10 bg-[#f8fafc] min-h-screen relative">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* 🌟 إذا كانت حالة السائق completed، نقوم بإخفاء الأزرار وإظهار شارة (Badge) تؤكد تفعيله */}
        {driverDetailes?.status === "completed" ? (
          <div className="flex items-center gap-2 bg-green-50 text-green-700 border border-green-200 px-5 py-2.5 rounded-2xl font-bold text-sm">
            <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
            <span>Ce chauffeur est déjà actif et vérifié ✅</span>
          </div>
        ) : (
          /* 🌟 إذا كانت الحالة ليست completed (مثلاً pending أو rejected)، تظهر أزرار التحكم طبيعياً */
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
            ...
            <div className="mt-8 space-y-3">
              <InfoRow
                icon={<Phone size={16} />}
                label="Téléphone"
                value={`+${driverDetailes?.phoneNumber || "..."}`}
              />
              <InfoRow
                icon={<MapPin size={16} />}
                label="Localisation"
                value={`${driverDetailes?.ville || "..."}, ${driverDetailes?.pay || ""}`}
              />
              <InfoRow
                icon={<Calendar size={16} />}
                label="Inscription"
                value={
                  driverDetailes?.createdAt
                    ? new Date(driverDetailes.createdAt).toLocaleDateString()
                    : "..."
                }
              />
            </div>
          ...
          <div className="bg-slate-900 rounded-[2rem] p-6 text-white shadow-xl relative overflow-hidden group">
            ...
            <p className="text-xs text-slate-400 leading-relaxed relative z-10">
              Veuillez vérifier scrupuleusement la validité des documents.
              Assurez-vous que les dates ne sont pas expirées et que les photos
              correspondent aux informations saisies.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {driverDetailes?.document
                // 1. تصفية المصفوفة لمنع الروابط الفارغة أو الـ null من المرور إلى الـ Image
                ?.filter((imgUrl) => {
                  const url = imgUrl as unknown as string;
                  return url && url.trim() !== "";
                })
                ?.map((imgUrl, index) => (
                  <div key={index} className="group flex flex-col gap-3">
                    ...
                    >
                      <Image
                        // 2. تمرير الرابط المضمون الآن بنسبة 100%
                        src={imgUrl}
