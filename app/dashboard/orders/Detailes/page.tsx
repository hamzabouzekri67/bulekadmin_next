...
export default function Detailes() {
 ...
 function getDistanceKm(lat1: number, lat2: number, lon1: number, lon2: number) {
  const R = 6371;
  ...
  const distanceM = distanceKm * 1000;
   return distanceKm < 1 ? distanceM.toFixed(0) + " M " : distanceKm.toFixed(0) + " km "
}
   ...
   const handleConfirm = (order: Order | null,) => {
    ...
    // }
    RejectedOrders({order,unavailableProducts ,user,indexMessage ,setNewOrders})
        ...
        {newOrders.map((order, index) => (
          ...
          >
            ...
            <div>
             ...
             {order.send && (
              ...
              <div className="flex justify-between"  onClick={() => handleOpenGoogleMaps(order)}>
                ...
                <p className="text-sm text-gray-500">
                 {getDistanceKm(order.restaurant.postionsEtabliss.coordinates[0],order.postionsClient.coordinates[0],order.restaurant.postionsEtabliss.coordinates[1],order.postionsClient.coordinates[1])}
                </p>
              </div>

              <div className="flex justify-between">
                <p className="text-sm text-gray-500">
                 Total commend complete
                </p>
            ...
            <div>
              ...
              <p className="text-md text-gray-500">
               {Number.parseInt(order.feedelivery) -order.offerfee} {order.currency}
              </p>
          ...
          {order.listOrder.map((listprod,indexProd)=>(
            ...
            <div key={indexProd} className="mb-3 border-b pb-2 last:border-b-0 last:pb-0">
             ...
             <div 
              className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 py-1"
            >
            ...
            {listprod.listSuplement && listprod.listSuplement.length > 0 &&
              listprod.listSuplement.map((c) => (
                <div key={c.id} className="text-xs sm:text-sm text-gray-500 italic">
                  {c.title} (x{c.qty})  -  {Number(c.price)*c.qty} {order.currency}
                </div>
              ))
             }
           ...
           <div className="flex justify-between">
            ...
            {order.diffDiscounted > 0 && (
             ...
             <div className="flex justify-between">
              ...
              <p className="text-md text-gray-500">
                {order.restaurantNetAmount - order.diffDiscounted} {order.currency}
              </p>
             ...
             {open && (
              <div className="fixed inset-0 z-50 flex items-end justify-center">
                ...
                <div className="relative w-full max-w-md bg-white rounded-t-3xl p-4 animate-sheetUp">
                  ...
                  <div className="mt-3 space-y-1 max-h-40 overflow-y-auto border-t pt-2">
                    ...
                    {order.listOrder.map((product) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={unavailableProducts.some(p => p.id === product.idproducts)}
                  ...
                  <button    onClick={()=>handleConfirm(order)} className="mt-4 w-full bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-xl font-black transition-all active:scale-95">
                    تأكيد الطلب
                    </button>
                    <button
                      onClick={() => setOpen(false)}
         ...
         <div className="flex justify-end space-x-2">
          ...
          <button className="bg-red-600 text-white text-sm py-1 rounded-md hover:bg-red-700 transition p-4" onClick={() => setOpen(true)}>
            Rejeter
          </button>

           <Link className="bg-gray-600 text-white text-sm py-1 rounded-md hover:bg-gray-700 transition p-4" href={{
