...
export default function MyOrders({ myOrders }: NewOrdersProps) {
  ...
  return (
    <div>
      ...
      {myOrders.length === 0 ? (
        <h1 className="px-2 text-gray-500">Vous n&apos;avez aucune commande en charge</h1>
      ) : (
        <div className="flex justify-between px-2 pt-2">
          
          <Link href='orders/myDetails'>
            <p className="mb-2 font-semibold text-gray-700">
              Vous gérez {myOrders.length} commande{myOrders.length > 1 ? "s" : ""}
            </p>
          </Link>
